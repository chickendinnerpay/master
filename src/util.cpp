...
const char * const BITCOIN_CONF_FILENAME = "chickendinner.conf";
const char * const BITCOIN_PID_FILENAME = "chickendinnerd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "chickendinner" is a composite category enabling all chickendinner-related debug output
            if(ptrCategory->count(string("chickendinner"))) {
...
#else
    const char* pszModule = "chickendinner";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\chickendinnerCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\chickendinnerCore
    // Mac: ~/Library/Application Support/chickendinnerCore
    // Unix: ~/.chickendinnercore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "chickendinnerCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/chickendinnerCore";
#else
    // Unix
    return pathRet / ".chickendinnercore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty chickendinner.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override chickendinner.conf
