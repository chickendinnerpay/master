 ...
 *
 *   // network
 *   CchickendinnerNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.chickendinner.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CchickendinnerNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CchickendinnerNetwork(UniValue objIn)
    {
...
// // can be under: chickendinnerNetwork
// //   -- signature requirements : Key1(User)
// class CchickendinnerNetworkVariable : public CGovernanceObject
// {
// private:
...
//     //     return (IsType() == chickendinnerNetwork);
//     // }
